...
####### end of connection ####
my_conn.execute("SELECT * FROM class9 limit 0,10")
...
i=1
recess = "RECESS"
for class9 in my_conn: 
    for j in range(len(class9)+1):
        if j < 5:
            e = Label(my_w,width=10,text=class9[j], borderwidth=2,relief='ridge',anchor='w')
            e.grid(row=i, column=j)
        elif j == 5:
            e = Label(my_w,width=10,text=recess[i-1], borderwidth=2,relief='ridge')
            e.grid(row=i, column=j)
        else:
            e = Label(my_w,width=10,text=class9[j-1], borderwidth=2,relief='ridge',anchor='w')
            e.grid(row=i, column=j)
        #e.insert(END, class12[j])
    i=i+1
def c(): os.system('py windows\\C9.py')
